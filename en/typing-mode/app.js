...
function applyTheme(theme = localStorage.getItem(KEYS.theme) || "light") {
  ...
  $$('[data-theme-toggle]').forEach(btn => btn.textContent = theme === "dark" ? "Light" : "Dark");
}
...
function startSession(item) { writeJSON(KEYS.session, { id: item.id || `custom-${Date.now()}`, title: item.title || "Untitled", text: item.text || "", source: item.source || "system" }); window.location.href = "workspace.html"; }
...
function renderLibrary(activeCategory = "All") {
  const filters = $('#categoryFilters'); const list = $('#libraryList'); if (!filters || !list) return;
  const categories = ["All", ...new Set(SYSTEM_TEXTS.map(item => item.category))];
  ...
  const items = activeCategory === "All" ? SYSTEM_TEXTS : SYSTEM_TEXTS.filter(item => item.category === activeCategory);
  list.innerHTML = items.map(item => `<article class="entry" data-id="${item.id}" tabindex="0" role="button" aria-label="Start ${escapeHTML(item.title)}"><h2 class="entry-title">${escapeHTML(item.title)}</h2><p class="hanzi-preview">${escapeHTML(item.text)}</p>${item.pinyin ? `<p class="muted">${escapeHTML(item.pinyin)}</p>` : ""}${item.translation ? `<p class="muted">${escapeHTML(item.translation)}</p>` : ""}</article>`).join("");
...
function bindCustomForm() {
  ...
  form.addEventListener('submit', event => { event.preventDefault(); warning.classList.remove('show'); const title = $('#customTitle').value.trim() || "Untitled practice"; const value = text.value.trim(); const custom = readJSON(KEYS.custom, []); if (!value) return showWarning("Please enter Hanzi text before saving."); if (value.length > 500) return showWarning("This entry is over 500 characters. Please shorten it."); if (custom.length >= 30) return showWarning("You already have 30 saved entries. Delete older texts before adding a new one."); custom.unshift({ id: `custom-${Date.now()}`, title, text: value, createdAt: Date.now(), source: "custom" }); writeJSON(KEYS.custom, custom); form.reset(); count.textContent = "0"; renderCustomTexts(); });
  $('#clearCustom')?.addEventListener('click', () => { if (confirm('Clear all saved custom texts?')) { writeJSON(KEYS.custom, []); renderCustomTexts(); } });
}
function renderCustomTexts() {
  const list = $('#customList'); if (!list) return; const custom = readJSON(KEYS.custom, []);
  if (!custom.length) { list.innerHTML = `<p class="empty">No saved custom texts yet.</p>`; return; }
  list.innerHTML = custom.map(item => `<article class="saved-item"><div class="item-top"><div><strong>${escapeHTML(item.title)}</strong><div class="date">${formatDate(item.createdAt)}</div></div><div><button class="ghost-btn start-custom" data-id="${item.id}" type="button">Practice</button><button class="danger-btn delete-custom" data-id="${item.id}" type="button">Delete</button></div></div><div class="snippet">${escapeHTML(item.text.slice(0, 80))}${item.text.length > 80 ? '…' : ''}</div></article>`).join("");
...
function renderHistory() {
  ...
  list.innerHTML = history.length ? history.map(item => `<article class="history-grid history-item"><span class="date">${formatDate(item.timestamp)}</span><strong>${escapeHTML(item.title)}</strong><span class="stat">${item.wpm ?? item.cpm ?? 0}</span><span class="stat">${item.accuracy}%</span><span class="stat">${item.errors}</span></article>`).join("") : `<p class="empty">No typing sessions yet.</p>`;
  $('#clearHistory')?.addEventListener('click', () => { if (confirm('Clear typing history?')) { writeJSON(KEYS.history, []); renderHistory(); } }, { once: true });
}
...
if (elapsedSeconds < 30) {
  ...
  $('#backHome').addEventListener('click', () => { if (active && !complete && !confirm('Your current session will be discarded. Go back to home?')) return; window.location.href = 'index.html'; });
...
function shareResult(result) {
  ...
  ctx.fillStyle = '#7b6d5d'; ctx.font = '400 34px Inter, Segoe UI, sans-serif'; ctx.fillText('Hanzi Recall Typing Result', 120, 185);
  ctx.fillStyle = '#2a241f'; ctx.font = '700 58px Inter, Segoe UI, sans-serif'; ctx.fillText(result.title || 'Typing Session', 120, 285);
  ...
  drawMetric(120, 350, 'WPM', result.wpm); drawMetric(780, 350, 'ACCURACY', `${result.accuracy}%`); drawMetric(120, 570, 'HANZI ERRORS', result.errors); drawMetric(780, 570, 'CORRECT HANZI / TOTAL', `${result.correct}/${result.total}`);
