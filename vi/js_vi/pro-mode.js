...
(function(){
  ...
  function loadQueue(){
    ...
    if ((!view || !view.length) && Array.isArray(App.state.masterVocabList) && App.state.masterVocabList.length) {
      ...
      state.fallbackNotice = 'Cấu hình ôn tập hiện tại không có từ, Pro Mode đã dùng danh sách tuần tự.';
    }
    ...
    if (!state.queue.length) {
      if (msg) msg.textContent = 'Hãy quay lại Trang chủ, chọn/kích hoạt một bộ từ vựng trước khi mở Pro Mode.';
      empty.hidden = false;
      focus.hidden = true;
    } else {
      empty.hidden = true;
  ...
  function updateStat(){
    ...
    if (statEl) statEl.textContent = `${state.learned} từ • ${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')}`;
  }
  ...
  function renderCurrent(){
    ...
    setFeedback(state.fallbackNotice || 'Enter để kiểm tra • Shift để nghe');
  }
  ...
  function submit(){
    ...
    if (!ans.trim()) {
      setFeedback('Hãy nhập đáp án trước khi Enter.', 'wrong');
      playWrong();
      pulse('bad');
      return;
    }
    ...
    if (ok) {
      ...
      state.correct++;
      state.learned++;
      setFeedback('Đúng. Tiếp tục!', 'correct');
      playCorrect();
      pulse('ok');
      ...
      state.wrong++;
      setFeedback('Sai rồi. Thử lại từ hiện tại.', 'wrong');
      playWrong();
      pulse('bad');
      focusInput();
    }
  ...
  function moveNext(opt = {}){
    ...
    if (state.currentIndex >= state.queue.length - 1) {
      setFeedback('Bạn đã hoàn thành phiên luyện tập.', 'correct');
      playCorrect();
      return;
    }
